...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.createSession = function(user_id, provider, req, customSessionLife) {
    var user;
    var newToken;
    var newSession;
    var password;
    ...
    return userDB.get(user_id)
      .then(function(record) {
        user = record;
        return generateSession(user._id, user.roles, customSessionLife);
      })
      .then(function(token) {
  ...
  function generateSession(username, roles, customSessionLife) {
    let sLife = customSessionLife || sessionLife;
    var getKey;
